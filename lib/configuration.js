...
export function set(newConfiguration, options = {}) {
    ...
    const env = (options.environment !== undefined) ? options.environment : 'default';

    if (options.assign) {
        configuration[env] = objectAssign(configuration[env], newConfiguration);
    } else {
...
export function get(key, fallbackValue) {
    ...
    let value = fetchFromObject(configuration['default'], key);

    if (currentEnvironment) {
        const envValue = fetchFromObject(configuration[currentEnvironment], key);
...
export function serialize(env) {
    ...
    return serializeJs(configuration[env]);
}
